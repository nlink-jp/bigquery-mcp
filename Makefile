MODULE  := github.com/nlink-jp/bigquery-mcp
BINARY  := bigquery-mcp
DIST_DIR := dist
...
.PHONY: build build-all package verify-release test live-test vet check clean help

## build: Build binary for the current OS/Arch → ./dist/bigquery-mcp
build:
	@mkdir -p $(DIST_DIR)
...
## test: Run all unit tests (fake BigQuery, no credentials)
test:
	go test ./...

## live-test: Run -tags live tests against a real project.
## Needs ADC and BIGQUERY_MCP_TEST_PROJECT (+ optional BIGQUERY_MCP_TEST_DATASET).
live-test:
	@test -n "$$BIGQUERY_MCP_TEST_PROJECT" || { echo "set BIGQUERY_MCP_TEST_PROJECT"; exit 1; }
	go test -v -tags live -timeout 10m ./internal/bq/... ./internal/tools/...
...
BREW_KIND := formula
BREW_DESC := Protection-first BigQuery MCP server with a dry-run budget gate and structured errors
